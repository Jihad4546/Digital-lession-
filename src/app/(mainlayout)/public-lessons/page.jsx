...
export default function PublicLessonsPage() {
  ...
  // চেক করুন ইউজার প্রিমিয়াম কি না বা এডমিন কি না
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 py-8 md:py-12 px-4 sm:px-6">
      <div className="max-w-7xl mx-auto">
        
        {/* Header Section */}
        <div className="text-center mb-10 md:mb-12">
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold bg-gradient-to-r from-pink-500 to-indigo-400 bg-clip-text text-transparent mb-3 px-2">
        ...
        {/* Lessons Grid */}
        {lessons.length === 0 ? (
          <div className="text-center text-slate-500 py-12 text-sm">No public lessons found.</div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {lessons.map((lesson) => {
              const isPremiumLesson = lesson?.accessLevel === "Premium";
              const isOwner = session?.user?.email === lesson?.creatorEmail;
              // যদি লেসন প্রিমিয়াম হয় এবং ইউজার প্রিমিয়াম না হয়, আর সে যদি লেসনের মালিকও না হয় -> ব্লার হবে
              const shouldBlur = isPremiumLesson && !isUserPremium && !isOwner;

              return (
                <div
                  key={lesson?._id}
                  className="relative flex flex-col justify-between border border-white/5 bg-slate-900/40 rounded-2xl p-5 md:p-6 backdrop-blur-sm overflow-hidden min-h-[320px] transition-all duration-300 hover:border-pink-500/20 hover:shadow-lg hover:shadow-pink-500/5"
                >
                  ...
                  {shouldBlur && (
                    <div className="absolute inset-0 z-20 backdrop-blur-md bg-slate-950/75 flex flex-col items-center justify-center p-4 text-center animate-in fade-in duration-300">
                      <div className="w-11 h-11 rounded-full bg-pink-500/10 flex items-center justify-center mb-3 border border-pink-500/30">
                        <FaLock className="text-pink-500 text-base" />
                      </div>
                  ...
                  <div>
                    ...
                    {/* Title */}
                    <h3 className="text-base md:text-lg font-bold text-white line-clamp-1 mb-2 hover:text-pink-400 transition-colors、">
                      {lesson?.title}
                    </h3>
                  ...
                  <div className="mt-4 pt-4 border-t border-white/5 flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-2.5 min-w-0">
                      <Image
                        ...
                        src={lesson?.creatorImage && lesson.creatorImage.startsWith("http") ? lesson.creatorImage : "/default-avatar.png"}
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[10px] text-slate-500 flex items-center gap-1" suppressHydrationWarning>
                          <FaCalendarAlt className="text-[9px]" /> 
                          {lesson?.createdAt ? new Date(lesson.createdAt).toLocaleDateString('en-GB') : "Recent"}
                        </p>
                      </div>
                    </div>
                    ...
                    {/* Action Button */}
                    {!shouldBlur && (
                      <Link href={`/lessons/${lesson?._id}`} className="shrink-0">

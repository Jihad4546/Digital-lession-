...
import { 
  ...
  FaFlag, FaShareAlt, FaComment, FaCalendarAlt, FaBrain 
} from "react-icons/fa";
...
export default function LessonDetailsPage() {
  ...
  // session এবং এর loading স্টেট (যদি আপনার authClient সাপোর্ট করে, নাহলে শুধু session চেক করলেই হবে)
  ...
  // ─── ০. AUTHENTICATION GUARD ───────────────────────────────────
  // ইউজার লগইন না থাকলে তাকে রিডাইরেক্ট করবে
  useEffect(() => {
    // যদি সেশন চেক করা শেষ হয় এবং সেশন ডেটা না থাকে (null হয়)
    if (session === null && !isPending) {
      toast.error("এই পেজটি দেখতে প্রথমে লগইন করুন।");
      router.push("/login");
    }
  ...
  useEffect(() => {
    if (!lessonId) return;
    // ইউজার লগইন না থাকলে ডেটা ফেচ করার দরকার নেই
  ...
  // সেশন লোড হওয়া পর্যন্ত অথবা ডেটা আসা পর্যন্ত লোডার দেখাবে
  if (loading || isPending || !session) {
    return (
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 py-8 md:py-12 px-4 sm:px-6 md:px-8 relative">
      <div className="max-w-4xl mx-auto space-y-6 md:space-y-8">
        ...
        <div className="border-b border-white/5 pb-6">
          ...
          <h1 className="text-2xl sm:text-3xl md:text-5xl font-extrabold bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent mb-4 leading-tight">
            {lesson?.title}
          </h1>

          {/* Featured Image */}
          {lesson?.featuredImage && (
            <div className="w-full h-48 sm:h-64 md:h-96 relative rounded-2xl overflow-hidden my-4 border border-white/5 shadow-inner">
        ...
        <div className="relative min-h-[100px]">
          {accessDenied ? (
            <>
              <div className="select-none blur-md opacity-20 space-y-4 pointer-events-none text-justify text-base md:text-lg leading-relaxed">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center bg-gradient-to-t from-slate-950 via-slate-950/90 to-transparent p-4 rounded-2xl">
                <div className="text-4xl md:text-5xl mb-3 animate-bounce">👑</div>
                <h3 className="text-xl md:text-2xl font-bold text-amber-400">Premium Content Locked</h3>
                <p className="text-xs md:text-sm text-slate-400 max-w-sm mt-2 mb-6">Upgrade to Premium to read full story insights.</p>
                <Button color="warning" className="font-bold px-6 md:px-8 shadow-lg bg-amber-500 text-slate-950" onPress={() => router.push("/pricing")}>
                  Upgrade Membership ✦
                </Button>
              </div>
            </>
          ) : (
            <div className="text-slate-300 leading-relaxed text-base md:text-lg whitespace-pre-line text-justify">{lesson?.description}</div>
          )}
        </div>
        ...
        {!accessDenied && (
          <>
            {/* 2. Lesson Metadata */}
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 bg-slate-900/40 p-4 rounded-xl border border-white/5 text-xs text-slate-400">
              <div className="flex items-center gap-1" suppressHydrationWarning>📅 Published: <b className="text-white ml-0.5">{lesson?.createdAt ? new Date(lesson.createdAt).toLocaleDateString('en-GB') : "N/A"}</b></div>
              <div suppressHydrationWarning>🔄 Updated: <b className="text-white ml-0.5">{new Date(lesson?.updatedAt || lesson?.createdAt || Date.now()).toLocaleDateString('en-GB')}</b></div>
              <div>👁️ Views: <b className="text-white ml-0.5">{viewsCount}</b></div>
              <div>🔒 Visibility: <b className="text-white ml-0.5">{lesson?.visibility || "Public"}</b></div>
            </div>

            {/* 4 & 5. Interaction Buttons & Stats */}
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-t border-b border-white/5 py-4 gap-4">
              <div className="flex flex-wrap items-center gap-4 md:gap-6 w-full sm:w-auto justify-between sm:justify-start">
                <button onClick={handleLikeToggle} className="flex items-center gap-2 hover:text-rose-400 transition-colors group">
                  {isLiked ? <FaHeart className="text-rose-500 text-lg md:text-xl scale-110" /> : <FaRegHeart className="text-lg md:text-xl group-hover:scale-110 transition-transform" />}
                  <span className="text-xs md:text-sm font-medium">{likesCount} Likes</span>
                </button>
                <button onClick={handleFavoriteToggle} className="flex items-center gap-2 hover:text-blue-400 transition-colors group">
                  {isSaved ? <FaBookmark className="text-blue-500 text-lg md:text-xl scale-110" /> : <FaRegBookmark className="text-lg md:text-xl group-hover:scale-110 transition-transform" />}
                  <span className="text-xs md:text-sm font-medium">{favoritesCount} Favorites</span>
                </button>
                <div className="relative">
                  <button onClick={() => setShareDropdownOpen(!shareDropdownOpen)} className="flex items-center gap-2 hover:text-green-400 transition-colors">
                    <FaShareAlt className="text-lg md:text-xl" /> <span className="text-xs md:text-sm font-medium">Share</span>
                  </button>
                  {shareDropdownOpen && (
              ...
              <button onClick={() => setReportModalOpen(true)} className="text-xs md:text-sm text-slate-500 hover:text-red-400 flex items-center gap-1 transition-colors ml-auto sm:ml-0">
                <FaFlag size={11} /> Report
              </button>
            </div>

            {/* 3. Author Card */}
            <Card className="p-4 bg-slate-900/40 border border-white/5 text-white flex flex-col sm:flex-row items-center sm:items-center gap-4 shadow-md backdrop-blur-sm w-full">
              <Avatar src={lesson?.creatorImage || ""} name={lesson?.creatorName || "Author"} size="lg" className="border border-white/10 shrink-0" />
              <div className="flex-1 text-center sm:text-left">
                <h4 className="font-bold text-base text-slate-200">{lesson?.creatorName || "Anonymous Learner"}</h4>
                <p className="text-xs text-slate-400 flex items-center justify-center sm:justify-start gap-1 mt-0.5"><FaCalendarAlt size={10} /> Created insights safely</p>
              </div>
              <Button size="sm" variant="bordered" className="text-xs text-slate-300 border-white/10 hover:bg-white/5 w-full sm:w-auto" onClick={() => router.push(`/dashboard/user/profile`)}>
                View All Lessons
              </Button>
            </Card>
            ...
            <div className="space-y-4 md:space-y-6 pt-4">
              <h3 className="text-lg md:text-xl font-bold flex items-center gap-2 text-slate-200">
                ...
                {comments.length === 0 ? (
                  ...
                  comments.map((comment) => (
                    <div key={comment._id} className="p-4 bg-slate-900/20 border border-white/5 rounded-xl flex gap-3 shadow-sm hover:border-white/10 transition-colors animate-in fade-in duration-300">
                      <Avatar src={comment.userImage} size="sm" className="border border-white/5 shrink-0" />
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2" suppressHydrationWarning>
                          <span className="text-xs font-semibold text-purple-400 truncate">{comment.userName}</span>
                          <span className="text-[10px] text-slate-500">{comment.createdAt ? new Date(comment.createdAt).toLocaleDateString('en-GB') : ""}</span>
                        </div>
                        <p className="text-sm text-slate-300 mt-1 leading-relaxed break-words">{comment.text}</p>
                      </div>
                    </div>
                  ))
                )}
            ...
            {recommended.length > 0 && (
              <div className="space-y-4 pt-8 border-t border-white/5">
                <h3 className="text-lg md:text-xl font-bold text-purple-400">📖 Recommended Lessons</h3>
